...
# All data gets compiled into a single string and sent through UART to be Read and Transmitted through LORA to a Gateway to be Displayed on Netmore.
...
#Initialise Pin 29 (GPIO22) to Go Low. When Script is completed we will send it High to put the Nano Power Timer to Sleep
...
app_key = '624FC57A0B2B7ADE1266E73FDF6CD939'
...
def set_app_key(app_key):
    if app_key is None or app_key == 'None':
        print('\nGenerate an AppKey on cloud.Netmore. and enter it at the top of this script to proceed')
        exit()
...
def join_netmore():
    while True:
        ...
        if 'joined' in data.split():
            status = 'connected'
            break
        if 'failed' in data.split() or len(data) == 0:
            print('Join Failed')
            pass
        sleep_ms(15000)
...
#Configure the device to trasmit correctly
test_uart_connection()
get_eui_from_radio()
set_app_key(app_key)
configure_regional_settings(band=band, DR='0', channels=channels)

# wrap the join_netmore() function in a try/except block
...
send_hex(Final_All_Data)

#Give a little buffer of time to make everything happy and ensure message sent successfully
sleep_ms(2000)

#Set Pin Value of Pin29 to High so that way the Nano Timer knows it is time to go to sleep (Thus saving power)
GPIO_22.value(1)
